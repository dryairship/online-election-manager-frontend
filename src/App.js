...
function App() {
	...
	const [ecAuthorised, setEcAuthorised] = useState(false);
	...
	const onInit = () => {
		if (localStorage.getItem('ecpass') === 'sparsh') {
			setEcAuthorised(true);
		...
		>
			{ecAuthorised ? (
				<>
					...
					{!showingResults && user && <Home user={user} onShowResults={onShowResults} />}
					{!showingResults && !user && <LRVTabbedPane onLogin={onLogin} />}
				</>
			) : (
				<h1>Contact Election Commission at 8529434655</h1>
			)}
